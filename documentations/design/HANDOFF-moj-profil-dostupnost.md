...
## 0. Amandman 2026-08-09 — četiri ispravke pre implementacije

Ovaj handoff se na četiri mesta sudarao sa obavezujućom dokumentacijom ili sa
stvarnom šemom. CTO je te tačke razrešio; **gde se ovaj amandman i tekst ispod
razlikuju, amandman pobeđuje.**

### 0.1 Profil se NE deli po formatu (menja §4, §5.1, §6)

`availability_profiles` **nema** `format` ni `location_id`, a unique ključ je
`(organization_id, therapist_profile_id, mode)` — dva `hourly_grid` profila
(jedan „online", jedan „uživo Niš") nisu moguća i neće se uvoditi.

Format je **capability usluge**, a ne osobina vremenske mreže. Efektivni format
koji korisnik može izabrati je presek:
...
| Model | Na koje pitanje odgovara |
|---|---|
| `Service` | Da li se usluga pruža online, uživo ili oba? |
| `TherapistServiceOffering` | Da li konkretan terapeut pruža tu uslugu, u kom formatu, gradu, trajanju i BookingMode-u? |
| `AvailabilityProfile` | **Kako** se generiše vreme: mod, korak mreže, zona, podrazumevane vremenske politike |
| `AvailabilityRule` | **Kada** je terapeut dostupan i koji kanal/lokacija važe u tom intervalu |
| `Slot` / occurrence | Izvedena jedinica — format **nasleđuje**, ne definiše |

**Posledica za UI:** segmentirani izbor iznad liste **ne prebacuje profile**. Sve
što je vezano za format radi se filtriranjem ponuda i termina po izabranom
formatu. Oznaka `· online` / `· uživo Niš` na kartici 1 čita se sa **pravila**.

`min_lead_time_hours` i `cancellation_notice_hours` na profilu su
**podrazumevane vrednosti**, ne konačne: lanac je
`channel override → offering override → profil → organizacija`. Rok otkazivanja
je poslovna politika usluge; kada `Appointment` nastane, čuva verziju politike
koja je tada važila.

### 0.2 Kartica 4 je read-only R4 šav (menja §4 karticu 4 i §7.6)

`company_reserved_capacity` ne postoji, a `BDS-013` drži same podatke o
kapacitetu za **R4**. Zato §7.6 („bez „Uskoro" placeholder-a") **ne važi za
karticu 4**: ona sme i treba da nosi razvojni bedž **„Planirano za R4"**.

Rezervisani kapacitet je **posebna vrsta pravila dostupnosti** — nije zauzet
termin, nije odsustvo, nije običan blok. `BDS-013` treba da definiše:

```
capacityScope: public | company_program | internal
```

Za ovu karticu: `capacityScope: company_program`, `publicVisibility: hidden`.

**Terminologija se menja.** Umesto „Nedostupno javnom bookingu" ide
**„Ne prikazuje se u javnom zakazivanju"** — „nedostupno" zvuči kao da je termin
zauzet ili da terapeut ne radi, a kapacitet je dostupan, samo određenoj grupi.

Arhitektonska granica koja važi već sada: rezervisani interval **troši stvarni
kapacitet terapeuta** — javni slotovi se u njemu ne generišu, online i uživo
opcije dele isti kapacitet, a Booking Engine ostaje autoritet za vremenski
konflikt i kada R4 doda kompanijska prava.

### 0.3 Razlog izuzetka: lista + napomena + vidljivost (menja §4 karticu 3 i §5.3)

Ne slobodan tekst. Tri polja:

- **`reason_code`** — kontrolisana lista; određuje podrazumevanu vidljivost.
- **`note`** — slobodan tekst za detalj („Konferencija · Beograd"). **Uvek
  interno**, nikada se ne prikazuje klijentu.
- **`client_visible`** — po zapisu. Godišnji odmor je podrazumevano vidljiv;
  terapeut može označiti i duže odsustvo kao vidljivo, da klijent zna da neće
  biti dostupan neko vreme i **da razlog nije prestanak rada**.

Klijent nikada ne vidi `note` — samo period i neutralnu oznaku razloga.

### 0.4 Imenovane klase i tokeni ne postoje (menja §3)

Projekat je **Tailwind v4 bez config fajla**; svi tokeni su u
`src/app/globals.css` `@theme`. Ne postoje: `.card`, `.eyebrow`, `.btn-outline`,
`.btn-primary`, ni tokeni `st-ok` / `st-wait` / `st-danger`.

| Handoff imenuje | Koristi se |
|---|---|
| `.card` | `rounded-card border-line bg-surface border px-6 py-6` |
| `.eyebrow` | inline `text-sage text-[11.5px] font-semibold tracking-[0.14em] uppercase` — **ne** `Eyebrow` komponenta (12.5px/.16em) |
| `.btn-outline` | `border-line-strong text-forest hover:border-coffee/40 rounded-full border bg-transparent px-4 py-2.5 text-[13px] font-semibold` |
| `rounded-panel` (§3) | `rounded-card` (20px) — vizuelna referenca ima 20px |
| `st-ok` / `st-wait` / `st-danger` | `badge-ok` / `badge-wait` / `badge-danger` (+ `-bg` parnjaci) ili `StatusBadge` |

### 0.5 Uređivanje tuđe dostupnosti (sužava §8)

`org_admin` za sada **samo čita** tuđu dostupnost. Puna izmena dolazi kasnije
kroz zasebnu admin rutu `/radni-prostor/tim/[memberId]`, ne kroz `/profil`.
`/radni-prostor/profil` ostaje **jedna ruta** čiji sadržaj zavisi od
capabilities iz server-side konteksta, ne od uloge u URL-u.
