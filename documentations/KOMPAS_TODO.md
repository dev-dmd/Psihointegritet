...
- [ ] **K3A.4** Završiti stvarni `.docx → RichDoc → segmenti → metadata → review → approve → publish → public article` tok. **DOCX uvoz je odložen do završetka UX-0A i UX-0B (D-062); tehnički obim ostaje nepromenjen.** Importer ubrzava unos u interfejs koji korisniku još ne objašnjava šta unosi, pa prvo ide razumljiv ručni tok. Ručno kreiranje članka stiže kroz UX-0B, nad postojećim `POST /content/entries`.
