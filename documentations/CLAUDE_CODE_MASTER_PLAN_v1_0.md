...
6. Any deviation from `ARCHITECTUAL/ARCHITECTURAL_RULES_REVISED.md` or `technical-documentation-architecture-v0.3.md` requires an ADR in `documentations/adr/` approved by the CTO.
7. End every milestone with the **completion report** defined in `ARCHITECTUAL/ARCHITECTURAL_RULES_REVISED.md` §24.
...
4. **`ARCHITECTUAL/ARCHITECTURAL_RULES_REVISED.md`** — coding contract: quality, optimization, best practice
...
5. `documentations/ARCHITECTUAL/ARCHITECTURAL_RULES_REVISED.md` (coding contract)
...
**Archived 2026-07-17** (in `documentations/archive/`, each with a SUPERSEDED banner explaining what replaced it) — never take decisions from these: `PRODUCT_CONTEXT.md` (was authority #2; stale T1/T2 terminology and a competing authority order — its role is now covered by Proposal v1.1 §3 + section 1 below; its §17 topic seed map survives only as a _draft_ source for R1.1), `production-plan.md` (M0–M6, superseded per IZMENE §6), `FOUNDATION_SETUP.md`, `CLAUDE_FOUNDATION_PROMPT.md`, `quality-gate-before-production.md` (replaced by section 12), `deployment-and-environments.md` (replaced by section 14), `technical-documentation-architecture-v0.1-SUPERSEDED.md` (Next-only/Drizzle/Auth.js).
...
| T3  | Qualifications              | **Settled 2026-07-26 by D-042.** All three are certified gestalt therapists. „Pod supervizijom" is outdated wording and must never reappear for anyone, including Marjan, whose submitted bio still carried it. Published titles: Anja — „Osnivačica Psihointegriteta · Socijalni radnik i sertifikovana geštalt psihoterapeutkinja"; Marija — „Pedagog i sertifikovana geštalt psihoterapeutkinja"; Marjan — „Psiholog i sertifikovani geštalt psihoterapeut". Certificate issuer and year are still unsupplied and are not displayed until a therapist provides them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| T9  | Language                    | Public content: Serbian Latin, **ekavica** as the site-wide default — aligned with Proposal v1.1 §3 („bračno savetovanje", „psihoterapijsko savetovanje") and the Engines document, both authored 2026-07-15. **Corrected 2026-07-17** (CTO decision): this row previously said „ijekavica … the owner's authored style", which contradicted the very proposal the client signed off on. **Refined 2026-07-18 (D-017), per-person exception:** Anja Stamenković personally speaks/writes **ijekavica** (from Prijedor) — her direct quotes and first-person bio text stay ijekavica; Marija Stamenković and Marjan Janković speak/write **exclusively ekavica**. Fixed product/service names (T1/T2, e.g. „Bračno savetovanje") never change per therapist — they are terminology, not personal speech. ⚠️ The rest of `frontend/src/content/homepage.ts` and several section components (hero, footer, workshop, faq, resources, support-paths) are still ijekavica — the sweep is R0.2 (see TODO.md D17); Anja's personal content in `content/therapists.ts` is already correct and is exempt from that sweep. Data model must not block later sr-Cyrl/en locales (`next-intl` already installed). **⚠️ Amended 2026-08-11 and 2026-08-13 (D-077):** this row now describes **one tenant**, not the platform. `PLATFORM_DEFAULT_LOCALE` is **`en`**; `SUPPORTED_UI_LOCALES = ["en", "sr-Latn"]`; `organizations.ui_locale` is the only organization-scoped render locale, while `organizations.default_content_locale` is only the creation stamp for new tenant-authored CMS content. Language never comes from `Accept-Language`. Psihointegritet stays `sr-Latn` with zero visual or content regression. The D-017 ijekavica exception left with the outgoing team (D-074/075), so `src/` is ekavica without exception; ekavica and ijekavica remain **content inside `sr-Latn`**, never separate platform locales, and are never auto-converted. Active delivery plan: `documentations/i18n/03_I18N_DEMO_CONTENT_IMPLEMENTATION_PLAN_v1_0.md`. |
...
**Backend:** FastAPI modular monolith, Python 3.14.6/uv, SQLAlchemy 2 async, Alembic, PostgreSQL 18.4, Redis 8.8, Railway, port 8001, OpenAPI export script. **Verified present 2026-07-17** (R0.3): the foundation is real and correct, but it is _only_ a foundation — all 11 module directories are empty, there are zero models, zero migrations and zero endpoints beyond the two health checks. See `documentations/TODO.md` §4 and §9.
...
| **R3**  | Content Engine / CMS + Knowledge & Resources (library, access grants)         | Faza 3                  | E3 (content part)    | 5–7 weeks                   | gated                     |
| **R4**  | Workshops, Programs & B2B                                                     | Faza 4                  | E4 (programs part)   | 6–8 weeks                   | gated                     |
| **R5**  | Online sessions, admin messaging, payments, packages, credits & subscriptions | Faza 5                  | E4/E5 (payment part) | 5–7 weeks                   | gated                     |
...
**Pre-R2 decision gate (R1.6 is internal document ordering only):** `Pre-R2 Booking Engine Decision Specification` is a documentation-only gate before R2 planning. It does not extend the R1 production-launch scope and does not rename formal R2, which remains the Operational MVP + Booking Engine.
...
**R0.1 — Documentation alignment** — _partly done 2026-07-17; live status in `documentations/TODO.md` §4_
...
- Remove any „pod supervizijom" string wherever it appears. Per D-042 the confirmed title lines state certification („sertifikovana geštalt psihoterapeutkinja" / „sertifikovani geštalt psihoterapeut") and are cleared for publication; only the certificate issuer and year remain unsupplied.
...
**R0.3 — Backend verification** — _verified 2026-07-17; findings in `documentations/TODO.md` §4 and §9_
...
**Purpose:** establish the typed, static contract that protects public content, UI/UX and discoverability before a CMS exists. The current TypeScript content becomes the first provider rather than throwaway data.

**In scope:** content models and status enums; static content-provider interface; allowed templates, section/card limits and CTA action registry; publication/approval validators and pure publish-gate evaluators; metadata/canonical/JSON-LD contracts; sitemap/robots/redirect contracts; read-only Content Health checks; unit tests, `npm run content:check` and a CI gate.

**Out of scope:** CMS database, FastAPI CMS CRUD, Control Center editor, reviewer accounts, audit log, revision persistence, scheduled publishing, production Booking Engine, payment systems and a Content Health dashboard.

**Non-negotiable rules:**

- `PublicationStatus` is `draft | in_review | approved | published | archived`; edits after approval create or return a new draft revision.
- Publication, availability, transaction mode and indexing are separate dimensions. A published informational page must not accidentally enable booking, registration or payment.
- `BookingMode` is `request | slot_request | disabled`. A user-selected slot is an `AppointmentRequest`, never a confirmed appointment.
- Approval capabilities are `clinical`, `legal` and `business`; they are requirements on content, not Clerk/FastAPI authorization roles.
- Widgets are enabled/disabled through a registry only. Their workflow, system destination and safety behavior are not CMS-editable.

**Definition of done:** the contracts in `CONTENT_*_v0.1.md` and `DISCOVERABILITY_CONTRACT_v0.1.md` are implemented against the static provider, CI rejects `error` findings, and public UI does not need to know whether data later comes from static TypeScript or R3 CMS.
...
**Explicitly out of R1:** accounts/dashboards, automatic slots, Google Calendar, payments, a full CMS application (database, FastAPI CRUD, editor and review accounts), protected resources, chat/video, workshops enrollment.
...
memberships           id, organization_id, user_id, role ENUM(owner,admin,therapist,content_editor),
                      status, UNIQUE(organization_id,user_id) — `content_editor` is reserved for R3; R2 ships no CMS editor/workflow
...
Until R3, catalog records above are operational booking references and mappings, not a CMS source of truth for public copy. R2 may configure fields needed for booking (service mapping, duration, buffers, availability), while R1.4.i static content remains the public provider. R3 owns public-content CRUD, editor, review and publish workflow.
...
**M2.2 — Operational catalog & availability (1–1.5 weeks)**
...
| ID  | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Blocks                                                 |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| S1  | **CLOSED 2026-07-26 (D-042)** — All three are certified; „pod supervizijom" is retired. Titles are published wording, not `draft`. Certificate issuer/year still unsupplied and not displayed.                                                                                                                                                                                                                                                                                             | nothing — no longer blocks                             |
...
_End of master plan v1.0. Update this document (and PRODUCT_DECISIONS.md) whenever a STOP item is resolved or a milestone is accepted._
