...
CONTENT_TAXONOMY_ROLE = sa.Enum(
    ...
    name="contenttaxonomyrole", native_enum=False, length=32,
)
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("revision_id", "term_id", "role", name="uq_content_revision_taxonomy_term"),
    )
    op.create_index(
        "ix_content_revision_taxonomy_terms_revision_id",
        "content_revision_taxonomy_terms", ["revision_id"],
    )
    op.create_index(
        "ix_content_revision_taxonomy_terms_term_id",
        "content_revision_taxonomy_terms", ["term_id"],
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["revision_id"], ["content_revisions.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["journey_intent_term_id"], ["taxonomy_terms.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["content_format_term_id"], ["taxonomy_terms.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["access_level_term_id"], ["taxonomy_terms.id"], ondelete="RESTRICT"),
...
def downgrade() -> None:
    ...
    op.drop_index("ix_content_revision_taxonomy_terms_term_id", table_name="content_revision_taxonomy_terms")
    op.drop_index("ix_content_revision_taxonomy_terms_revision_id", table_name="content_revision_taxonomy_terms")
