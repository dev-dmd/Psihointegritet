...
export async function listStaffAppointmentRequests(
...
export async function listStaffAppointments(
    params?: { therapist_id?: string; date_from?: string; date_until?: string },
): Promise<Appointment[]> {
    const qs = new URLSearchParams();
    if (params?.therapist_id) qs.set("therapist_id", params.therapist_id);
