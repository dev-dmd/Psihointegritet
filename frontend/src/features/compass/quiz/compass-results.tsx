...
export function CompassResults({
  ...
  const t = useTranslations("public.compass.results");
  const quiz = useTranslations("public.compass.quiz");
  return (
    <div className="flex min-h-0 flex-col">
      <header className="border-line shrink-0 border-b px-5 pt-2 pb-4 md:px-8">
        <div className="mx-auto flex max-w-[900px] items-start justify-between gap-4">
          <div>
            ...
            >
              {experience?.summary.title ?? t("preparing")}
            </h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label={quiz("close")}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6 md:px-8">
        <div className="mx-auto max-w-[900px]">
          {isLoading ? (
            <p className="text-coffee/70 py-10 text-center">{t("loading")}</p>
          ) : null}
          {isError ? (
