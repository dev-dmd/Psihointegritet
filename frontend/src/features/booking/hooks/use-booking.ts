...
export const bookingQueryKeys = {
    ...
    slots: (params: SlotQueryParams) => [...bookingQueryKeys.all, "slots", params] as const,
    clientRequests: (email: string) => [...bookingQueryKeys.all, "client-requests", email] as const,
};
...
export function useHoldSlot() {
    return useMutation({
        mutationFn: ({ payload, signal }: { payload: SlotHoldRequest; signal?: AbortSignal }) =>
...
export const staffBookingQueryKeys = {
    ...
    appointments: (params?: { therapist_id?: string; date_from?: string; date_until?: string }) =>
...
export function useStaffAppointmentRequests(
    params?: { therapist_id?: string; status?: string },
) {
    const { listStaffAppointmentRequests } = require("@/lib/api/booking");
    return useQuery({
