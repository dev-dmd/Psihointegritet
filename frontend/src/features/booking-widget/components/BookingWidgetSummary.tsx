        ...
        <div className="mt-6">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <p
                className={cn("text-sm font-medium sm:text-[15px]", theme.body)}
              >
          ...
          {availableServices.length > 0 ? (
            <div className="scrollbar-hide mt-4 flex h-[76px] flex-wrap items-start gap-2 overflow-y-auto">
              <label
                className={cn(
              ...
              {availableServices.map((svc) => (
                <button
                  ...
                  className={cn(
                    "h-[30px] rounded-full border px-3 py-1 text-[12px] font-medium transition-all",
                    theme.border,
                    theme.muted,
                    "hover:border-sage cursor-pointer hover:text-current",
                  )}
                >
                  {svc.name}
                  <span className="ml-1.5 opacity-60">
                    {svc.durationMinutes}′ ·{" "}
                    {formatBookingPrice(svc.price, svc.currency)}
                  </span>
                </button>
              ))}
            </div>
          ...
          {otherTherapists.length > 0 ? (
            <div>
              <label
                className={cn(
                  "w-full text-[13px] font-semibold tracking-[0.08em] uppercase",
                  theme.muted,
                )}
              >
                Ostali terapeuti
              </label>
              <div className="scrollbar-hide mt-3 flex h-[76px] flex-wrap items-start gap-2 overflow-y-auto">
                {otherTherapists.map((t) => (
                  <button
                    ...
                    className={cn(
                      "inline-flex h-[30px] items-center gap-1.5 rounded-full border px-3 py-1 text-[12px] font-medium transition-all",
                      theme.border,
                      theme.muted,
                      "hover:border-sage cursor-pointer hover:text-current",
                    )}
